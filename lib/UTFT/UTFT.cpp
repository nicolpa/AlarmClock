...
UTFT::UTFT() : backlightOn(false), doFade(false), fadeTime(DEFAULT_FADE_TIME), brightness(DEFAULT_BRIGHTNESS)
{
}

UTFT::UTFT(byte model, int RS, int WR, int CS, int RST, int SER)
	: backlightOn(false), doFade(false), fadeTime(DEFAULT_FADE_TIME), brightness(DEFAULT_BRIGHTNESS)
{ 
...
void UTFT::InitLCD(byte orientation, bool _backlightOn)
{
	orient=orientation;
	_hw_special_init();
...
void UTFT::printNumF(double num, byte dec, int x, int y, char divider, int length, char filler)
{
	...
	if (filler != ' ')
	{
		...
		{
			st[0]='-';
			for (unsigned long i = 1; i < sizeof(st); i++)
				if ((st[i]==' ') || (st[i]=='-'))
					st[i]=filler;
		}
		else
...
void UTFT::lcdOn()
{
	if(this->doFade)
	{
		...
		for(byte br = 0; br < this->brightness; br++)
		{
			br++;
			analogWrite(BACKLIGHT_PIN, br);
			delay(brakeTime);
...
void UTFT::setFadeTime(uint32_t _fadeTime)
{
	this->fadeTime = _fadeTime;
}

void UTFT::fade(bool _doFade)
{
	this->doFade = _doFade;
}
