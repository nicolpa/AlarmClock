...
namespace lcd
{
	...
	// 	Color(byte red, byte, byte green, byte blue)
	// 	{
	// 		this->red = red;
	// 		this->green = green;
	// 		this->blue = blue;
	...
	public:
		...
		void InitLCD(byte orientation = LANDSCAPE, bool _backlightOn = true);
		void clrScr();
		...
		byte getBrightness();
		void setFadeTime(uint32_t _fadeTime);
		void fade(bool _doFade);
